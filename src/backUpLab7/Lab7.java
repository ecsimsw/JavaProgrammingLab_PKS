...
public class Lab7 {
		...
		do {
			...
			if(key == 'p') { // p-key
				...
				if(index < plist.size() - 1) {
					PeriodicElement next = plist.getNext(index);
	...
	public static void main(String[] args) {
		...
		// get 10 from ptable
		PeriodicElement[] result = ptable.getElements(0, 10);		
		// convert array to arraylist
		//List<PeriodicElement> list = new ArrayList<>(Arrays.asList(elements));
		System.out.println("\nplist");
		PeriodicTableList plist = new PeriodicTableList();
		for (PeriodicElement pe : result) {
			plist.add(pe);
		}
		plist.print();
		...
		// get 5 from ptable
		PeriodicElement[] result2 = ptable.getElements(10, 15);
		for (PeriodicElement pe : result2) {
		...
		for (IPeriodicElementFinder finder : finders) {
			...
			List<PeriodicElement> foundElements = plist.find(finder);
			for (PeriodicElement pe : foundElements) {
				System.out.println(pe);			
			}
			System.out.println("\n\n");			
		}
		...
		// pmap add/remove/
		PeriodicElement element = ptable.getRandomElement();
		element.print();
		...
		for (IPeriodicElementFinder finder : finders) {
			System.out.println("Using " + finder.getClass().getName());
			List<PeriodicElement> foundElements = pmap.find(finder);
			for (PeriodicElement pe : foundElements) {
				System.out.println(pe);			
			}
			System.out.println("\n\n");			
